...
function pageinfo_editor_lines($db,$teamid=0,$league=false,$showDropdown=true){
	...
	if(isset($_POST["sbtUpdateLines"])){
		foreach($_POST["txtLine"] AS $line=>$name){
			$SQL = "UPDATE Team". $_REQUEST["League"] ."Lines SET " . $line . " = '" . $name . "' WHERE TeamNumber = " . $_REQUEST["TeamID"] . ";";
			$db->exec($SQL);
					...
					foreach($tabs AS $i=>$t){
						?><div id="tabs-<?= ++$count ?>" class="tabcontainer"><?
							if($i == "Forward"){	
								...
								make_blocks($row,$blocks,$positions,$strategy,$i,$availableplayers);
							}elseif($i == "PP" || $i == "PK4" || $i == "4VS4" || $i == "PK3"){
								$bcount = 0;
								...
								<div class="linesection <?= MakeCSSClass($i)?> goalies">
									<h4>Starting Goalie</h4>
									<div class="blockcontainer">
										<? $row["Goaler1"] = (isset($availableplayers[MakeCSSClass($row["Goaler1"])])) ? $row["Goaler1"]: "";?>
										<div class="startinggoalie"><?= "<input id=\"Goaler1\" onclick=\"ChangePlayer('Goaler1','". $league ."');\"  readonly type=\"text\" name=\"txtLine[Goaler1]\" value=\"". $row["Goaler1"] ."\">";?></div>
									</div>
									<h4>Backup Goalie</h4>
									<div class="blockcontainer">
										<? $row["Goaler2"] = (isset($availableplayers[MakeCSSClass($row["Goaler2"])])) ? $row["Goaler2"]: "";?>
										<div class="backupgoalie"><?= ($edit) ? "<input id=\"Goaler2\" onclick=\"ChangePlayer('Goaler2','". $league ."');\" readonly type=\"text\" name=\"txtLine[Goaler2]\" value=\"". $row["Goaler2"] ."\">" : Jersey($row["Goaler2"],$playerinfo) ;?></div>
									</div>
								</div>
								<?
								$field = array();
								$field["start"] = array("Forward"=>"Forwards","Defense"=>"Defensemen");
								$field["end"] = array("N1","N2","N3","PP1","PP2","PK","PP","PK1","PK2");
...
function make_blocks($row,$blocks,$positions,$strategy,$i,$availableplayers){
	...
	foreach($blocks[$i] AS $bid=>$block){
		...
		?><div class="linesection <?= MakeCSSClass($i)?> <?= MakeCSSClass($bid)?>">
			...
			<div class="blockcontainer">
				<div class="positionwrapper">
					<? foreach($positions[$i] AS $pid=>$pos){?>
					<div class="positionline">
						<div class="positionlabel"><?= $pos?></div>
						<div class="positionname">
							<? $row[$field . $pid] = (isset($availableplayers[MakeCSSClass($row[$field . $pid])])) ? $row[$field . $pid]: "";?>
